...
Windows PC 原型，版本 0.6.0-preview。原创的建造与异步攻城方向项目。
...
随后双击 `artifacts/WindowsPreview/Hearthhold.exe`，或解压 `artifacts/Hearthhold-0.6.0-native-win-x64.zip`。运行环境为 Windows x64 和 .NET Framework 4.x；本机已完成编译和交互验证，无需 Unity 或 .NET SDK。
...
构建后运行 `artifacts/WindowsUnity/Hearthhold.exe`，发布包为 `artifacts/Hearthhold-0.6.0-unity-win-x64.zip`。首次构建需要已安装、登录并激活的 Unity 编辑器；发布包运行时不需要安装 Unity。

第一次进入时，已有一座主城、两座金矿、一座晶露池、一座远征营、两座防御建筑和一段城墙，初始资源足够体验建造与升级。可以先收取产出、摆放建筑，再点右下角“出发远征”。

![Unity 0.6 基地画面](artifacts/screenshots/27-unity-home-v06.png)

## 本轮更新

- 0.6 新增战前编队与45点初始营位。每名兵种有独立占用：先锋1、游侠1、铁卫5、破城手2；升级或增加远征营会扩充容量。
- 新增原创训练队列：训练逐个完成并消耗金币，支持取消最后一个项目并全额返还；退出游戏后按真实离线时间推进，战斗期间暂停。
...
- 使用原创 3D 美术方向板约束比例、材质和特效语言，详见 [`docs/VISUAL-DIRECTION.md`](docs/VISUAL-DIRECTION.md)。方向板不进入运行时，也没有使用外部游戏资产。
...
0.6 兼容 0.1、0.2、0.4 和 0.5 存档。更新前退出游戏并备份 `Saves` 文件夹；在原目录替换程序即可继续原村庄，换目录则手动复制 `Saves`。旧存档首次读取时补齐空白战役记录并获得一套均衡初始编队；原有建筑、资源和超限建筑都会保留。本次开发测试没有修改用户正式存档。
...
**当前机器已使用 Unity 6000.6.0f1 / URP 17.6.0 实际完成 Windows x64 构建。** 构建日志包含 `Build Finished, Result: Success` 和本轮输出路径标记；四项运行时烟雾测试以独立测试存档启动程序，分别渲染 1440×900 聚落、战役进度、训练编队与交战画面并以退出码 0 结束。

Unity 客户端已接入新版网格、顶点色光照材质、拆除确认、建造上限、兵种图鉴、训练编队和首次出征引导。自动构建、四类验收画面与规则层10关通关已经验证；10关逐关人工试玩和持续性能测试仍待完成，不能把烟雾测试视为全部玩法验收。

![Unity 0.6 训练编队](artifacts/screenshots/29-unity-training-v06.png)

![Unity 0.6 交战效果](artifacts/screenshots/30-unity-battle-v06.png)
...
这一版是 0.6 可玩原型迭代，不是完整商业游戏。模型和特效完成了第二轮原创程序化升级，但仍未达到外包成品美术水准，也没有角色骨骼和动作制作。尚未实现：联网账号、匹配、服务端校验、部落系统、建筑升级计时队列、第二种法术、框选多建筑、完整音效、保存的战斗回放与设置持久化。
